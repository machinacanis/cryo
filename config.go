...
type Config struct {
	...
	EnablePluginAutoLoad         bool     `json:"enable_plugin_auto_load,omitempty,omitzero"`         // 是否启用插件自动加载
	...
	EnableCronScheduler          bool     `json:"enable_cron_scheduler,omitempty,omitzero"`           // 是否启用内置的gocron定时任务调度器
}
